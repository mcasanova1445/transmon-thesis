...
\begin{enumerate}
    ...
    \item Un metal en estado normal no es transparente, es decir, que el material permita el paso de onda electromagnética sin decaer, a la radiación visible e infrarroja.
\end{enumerate}
\item Superconductor:
\begin{enumerate}
...
\section{Componentes de la corriente en las uniones de Josephson}

La corriente en las uniones Josephson tiene tres componentes:
...
\section{Cuantización del oscilador armónico LC}

En la mecánica Hamiltoniana clásica, la evolución dinámica de un sistema clásico está descrito por un conjunto de variables canónicas conjugadas $r=(q,p)$ de la siguiente manera:
...
Donde el Hamiltoniano $H$ es la suma de la energía potencial $V$ y la energía cinética $T$, $H=T+V$.
...
Paul Dirac introdujo el método de analogía clásica para la cuantización de sistemas físicos \cite{Dirac_1925}. Este método consiste en cuantizar las teorías clásicas intentando mantener las estructuras formales de la teoría clásica en la mayor medida posible.
...
Luego se demostró que esta relación no aplica para todo sistema físico, sino que es un caso especial de una relación más general. Sin embargo, para los casos a estudiar en este trabajo, sí se cumple.

El procedimiento a seguir para cuantizar circuito LC será:

\begin{enumerate}
    \item Construir el Hamiltoniano clásico del sistema en términos de coordenadas canónicas.
    \item Sustituir los observables por operadores cuánticos.
    \item Forzar la relación de conmutación canónica
\end{enumerate}

Un ejemplo de variables canónicamente conjugadas son la carga y el flujo magnético. Por esto, podemos cuantizar un oscilador armónico LC con la cuantización canónica.
...
Esto demuestra que $\phi$ y $Q$ son coordenadas canónicas. Donde $\phi$ y $Q$ son posición y momentum generalizados, respectivamente. Así que procedemos a sustituirlas por los operadores $\hat{\phi}$ y $\hat{q}$ y escribimos el Hamiltoniano cuántico
...
Esto, además, significa que la carga en el capacitor y el flujo en el inductor no pueden ser medidos simultaneamente, pues el orden de las medidas afecta el resultado. Finalmente, la frecuencia de resonancia de este oscilador es $\omega = \frac{1}{\sqrt{L C}}$ y los niveles de energía se encuentran separados por $\hbar \omega$

\section{Qubits superconductores}

Los qubits superconductores se basan en circuitos osciladores no lineales, hechos a partir de uniones Josephson \cite{wendin}. Estos osciladores son anarmónicos, de manera que se pueda operar sobre un par específico de niveles de energía.

Primero, reescribimos el Hamiltoniano del circuito LC en términos de la cantidad de pares de Cooper y de la diferencia de fase en los extremos del inductor, en lugar de la carga y el flujo
...
Ahora introducimos el término de la unión Josephson, para considerar un circuito LCJ (Inductor - Condensador - Unión Josephson). Para esto, primero hallemos la expresión de la energía almacenada en la unión:

\begin{equation}
    \begin{array}{r c l}
        E_J &=& \int I_J V_J dt
    = \int I_0 \sin(\delta) \frac{\hbar}{2e} \frac{d\delta}{dt} dt \\
    &=& \frac{\hbar I_0}{2e} \int \sin(\delta) dt
    = - \frac{\hbar I_0}{2e} \cos(\delta) = - E_{J0} \cos(\delta) .
    \end{array}
\end{equation}

Donde $E_{J0} = \frac{\hbar I_0}{2e}$ se conoce como energía de Josephson. Con este término, el Hamiltoniano del circuito LCJ será:
...
En este Hamiltoniano también hemos separado los términos $n_g$ y $\delta_e$, los cuales representan la carga inducida por voltaje en el capacitor C (isla qubit) y $\delta_e$ es la fase inducida por flujo sobre la unión Josephson.

Para describir el sistema en términos de la ecuación de Schrödinger en función de la diferencia de fase $\delta$, se introduce $\hat{n}=-i\hbar \frac{\partial}{\partial \delta}$. Quedando la siguiente ecuación:

\begin{align}
    E_C (-i\hbar \frac{\partial}{\partial\delta}-n_g)^2+U(\delta))\psi &= E \psi \\
    U(\delta) &= -E_{J0} \cos(\delta)+E_L \frac{(\delta-\delta_e)^2}{2}
\end{align}

Variando el término $E_L$, tenemos dos casos principales. El primero, cuando $E_L$ tiende a cero, es decir, cuando la inductancia tiende a infinito y el circuito se convierte en uno CJ. En este caso, el potencial es de tipo coseno puro, y la solución toma la forma de funciones de Mathieu (las cuales se definen como la solución a esta ecuación). La dispersión de los niveles de energía, y su dependencia de $n_g$, dependerá del cociente $E_{J0}/E_C$. Por esto resulta de de especial interés el caso en el que la capacitancia $C$ es grande. El segundo caso principal es aquel en el que $E_L$ es del mismo orden de magnitud que $E_{J0}$. En este caso, el potencial deja de ser periodico y describe una parábola modulada por una función sinusoidal. La forma del fondo de la parábola modulada y la estructura de los niveles de energía dependerán altamente del cociente $E_L/E_{J0}$ y del flujo externo $\phi_e$.
